# 📚 CareCircle Complete Learning Guide
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   "Dad fell last Tuesday and was in ER. My sister didn't know for           │
│    6 hours because we're in different time zones. His medication            │
...
│   • Shared Google Doc is chaos                                              │
│   • Missed appointments, double-booked caregivers                           │
│   • Medication errors                                                       │
│   • Emergency info scattered everywhere                                     │
│   • 6-hour delay in critical communication                                  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────────┐
│                         THE SOLUTION: CARECIRCLE                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│                          ┌───────────────────┐                              │
│                          │    CareCircle     │                              │
...
│     ▼              ▼              ▼              ▼              ▼          │
│ ┌────────┐  ┌───────────┐  ┌───────────┐  ┌─────────┐  ┌───────────┐       │
│ │ Meds   │  │  Calendar │  │ Emergency │  │ Family  │  │ Documents │       │
│ │Tracking│  │Appointments│  │  Alerts  │  │  Team   │  │   Vault   │       │
│ └────────┘  └───────────┘  └───────────┘  └─────────┘  └───────────┘       │
│                                                                             │
│   ✅ One place for ALL care information                                     │
│   ✅ Real-time updates to all family members                                │
│   ✅ Emergency alerts in seconds, not hours                                 │
│   ✅ Medication logging & reminders                                         │
│   ✅ Caregiver shift coordination                                           │
│   ✅ Health timeline for doctor visits                                      │
│   ✅ Works offline (emergency info cached)                                  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│           ▼                       ▼                       ▼                 │
│   ┌───────────────┐       ┌───────────────┐       ┌───────────────┐        │
│   │  PostgreSQL   │       │   RabbitMQ    │       │    BullMQ     │        │
│   │   Primary     │       │   Exchange    │       │    Worker     │        │
│   │   + Replicas  │       │   + Queues    │       │   Processes   │        │
│   └───────────────┘       └───────────────┘       └───────────────┘        │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│   │     │   mutationFn: () => api.post('/medications/123/log', {       │ │  │
│   │     │     status: 'GIVEN',                                         │ │  │
...
│   │     │ • RolesGuard checks user has CAREGIVER or ADMIN role         │ │  │
│   │     │ • ValidationPipe validates request body                      │ │  │
│   │     │ • MedicationsController.log() called                         │ │  │
│   │     │ • MedicationsService.log() creates log entry                 │ │  │
│   │     │                                                              │ │  │
...
│   │     │   'medication.logged',                                       │ │  │
│   │     │   {                                                          │ │  │
│   │     │     medicationId, careRecipientId, status,                   │ │  │
│   │     │     loggedById, loggedByName                                 │ │  │
│   │     │   },                                                         │ │  │
│   │     │   `medication.${status.toLowerCase()}.${careRecipientId}`,   │ │  │
│   │     │   '/api/v1/medications/123/log'                              │ │  │
│   │     │ );                                                           │ │  │
│   │     │                                                              │ │  │
│   │     │ → Saved to outbox table (for reliability)                    │ │  │
│   │     │ → Immediately attempted to RabbitMQ                          │ │  │
│   │     └──────────────────────────────────────────────────────────────┘ │  │
│   │                              │                                       │  │
│   │                              ▼                                       │  │
│   │  4. EVENT CONSUMERS                                                  │  │
│   │     ┌──────────────────────────────────────────────────────────────┐ │  │
│   │     │ WEBSOCKET CONSUMER:                                          │ │  │
│   │     │ → Sends to family members via Socket.io                      │ │  │
│   │     │ → UI updates in real-time                                    │ │  │
│   │     │                                                              │ │  │
│   │     │ NOTIFICATION CONSUMER:                                       │ │  │
│   │     │ → Sends push notifications to family phones                  │ │  │
│   │     │ → "Sarah gave Dad his Metformin"                             │ │  │
│   │     │                                                              │ │  │
│   │     │ AUDIT CONSUMER:                                              │ │  │
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   CareCircle uses FAMILY-BASED multi-tenancy:                               │
│                                                                             │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                                                                     │   │
│   │   Family: "Thompson Family"                                         │   │
│   │   ├── Members:                                                      │   │
│   │   │   ├── Sarah Thompson (ADMIN)                                    │   │
│   │   │   ├── Mike Thompson (CAREGIVER)                                 │   │
│   │   │   └── Jennifer Thompson (VIEWER)                                │   │
│   │   │                                                                 │   │
│   │   └── Care Recipients:                                              │   │
│   │       └── Margaret "Grandma Maggie" Thompson                        │   │
│   │           ├── Medications                                           │   │
│   │           ├── Appointments                                          │   │
│   │           ├── Documents                                             │   │
│   │           ├── Timeline                                              │   │
...
│   ──────────────                                                            │
│                                                                             │
│   // Every query is scoped to the user's family!                            │
│                                                                             │
│   // WRONG (security risk!)                                                 │
│   const meds = await medicationRepo.find({                                  │
│     where: { careRecipientId }  // Could access ANY family's data!          │
│   });                                                                       │
│                                                                             │
│   // RIGHT (family-scoped)                                                  │
│   const meds = await medicationRepo.find({                                  │
│     where: {                                                                │
│       careRecipient: {                                                      │
│         familyId: user.currentFamilyId  // Scoped to user's family          │
│       }                                                                     │
│     }                                                                       │
│   });                                                                       │
│                                                                             │
...
│   WITHOUT Events:                                                           │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │ async logMedication(dto: LogMedicationDto) {                        │   │
│   │   const log = await this.medicationRepo.save(dto);                  │   │
│   │                                                                     │   │
│   │   // All of this runs IN THE REQUEST                                │   │
│   │   await this.sendPushNotifications(log);      // 200ms              │   │
│   │   await this.sendWebSocketUpdate(log);        // 50ms               │   │
│   │   await this.createAuditLog(log);             // 100ms              │   │
│   │   await this.updateDashboardCache(log);       // 75ms               │   │
│   │   await this.checkForInteractions(log);       // 150ms              │   │
│   │                                                                     │   │
│   │   return log;  // Total: 575ms+ response time! 😱                   │   │
│   │ }                                                                   │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│   WITH Events:                                                              │
...
│   │   await this.eventPublisher.publish(                                │   │
│   │     'medication.logged',                                            │   │
│   │     { medicationId: log.id, status: log.status, ... }               │   │
│   │   );                                                                │   │
│   │                                                                     │   │
│   │   return log;  // Response in ~20ms! 🚀                             │   │
│   │ }                                                                   │   │
│   │                                                                     │   │
│   │ // Everything else happens ASYNCHRONOUSLY via consumers             │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│                    │  ┌─────────────────┐    │                              │
│                    │  │ medication_logs │    │                              │
│                    │  │ event_outbox ←──┼────┼── 2. Write to outbox         │
│                    │  └─────────────────┘    │    (same transaction)        │
│                    └─────────────────────────┘                              │
│                                │                                            │
│                                │ 3. Outbox processor reads                  │
│                                ▼    every 10 seconds                        │
│                    ┌─────────────────────────┐                              │
│                    │    Outbox Processor     │                              │
│                    │    (Cron Job)           │                              │
...
│   └────────┬────────┘                 └────────┬────────┘  └──────┬──────┘  │
│            │                                   │                  │         │
│            │ 5a. Emit via Socket.io            │ 5b. Send push    │ 5c. Log │
│            ▼                                   ▼                  ▼         │
│   ┌─────────────────┐                 ┌─────────────────┐  ┌─────────────┐  │
│   │ Family Members  │                 │  Push Service   │  │ Audit Log   │  │
│   │    (Browser)    │                 │ (Web Push/FCM)  │  │  (DB/SIEM)  │  │
│   └─────────────────┘                 └─────────────────┘  └─────────────┘  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│   │ })                                                                  │   │
│   │ export class CareCircleGateway implements OnGatewayConnection {     │   │
│   │                                                                     │   │
│   │   // Map: familyId → Set of socket IDs                              │   │
│   │   private familyRooms = new Map<string, Set<string>>();             │   │
│   │                                                                     │   │
│   │   async handleConnection(client: Socket) {                          │   │
│   │     // 1. Verify JWT from cookie                                    │   │
│   │     const token = client.handshake.headers.cookie?.accessToken;     │   │
│   │     const user = await this.jwtService.verify(token);               │   │
│   │                                                                     │   │
│   │     // 2. Join user to their family's room                          │   │
│   │     client.join(`family:${user.familyId}`);                         │   │
│   │   }                                                                 │   │
│   │                                                                     │   │
│   │   // Called by WebSocket consumer when events arrive                │   │
...
│   │                                                                     │   │
│   │   useEffect(() => {                                                 │   │
│   │     const socket = io(`${API_URL}/carecircle`, {                    │   │
...
│   │       // Critical: show modal, play sound!                          │   │
...
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   We use Web Push API (VAPID) for push notifications:                       │
│                                                                             │
...
│   │       icon: data.icon,                                              │   │
│   │       badge: '/badge.png',                                          │   │
│   │       vibrate: [200, 100, 200],  // For emergencies                 │   │
│   │     })                                                              │   │
│   │   );                                                                │   │
│   │ });                                                                 │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
...
│   │   └── accept-invite/[token]/page.tsx  # /accept-invite/abc123           │
│   │                                                                         │
│   └── (app)/                  # App route group (protected)                 │
│       ├── layout.tsx          # App shell with sidebar/nav                  │
│       ├── page.tsx            # /dashboard (home)                           │
│       ├── care/               # Care recipient routes                       │
│       │   └── [id]/           # /care/cr_abc123                             │
│       │       ├── page.tsx    # Care recipient overview                     │
...
│   LAYOUT HIERARCHY:                                                         │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │ RootLayout (providers: React Query, Auth, Toast)                    │   │
│   │   │                                                                 │   │
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│  🏡 CareCircle Development Environment                                       │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  APPLICATIONS:                                                              │
